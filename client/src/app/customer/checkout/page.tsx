...
export default function CartCheckout() {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (
      ...
      (user.walletBalance || 0) < totals.finalTotal
    ) {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        ...
        clearCart();
        router.push(`/customer/orders/${res.data.data._id}`); // migrated order-tracking path
      }
    } catch (error: any) {
      ...
      <div className="flex items-center justify-between border-b border-border pb-6">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">
            Ordering from{" "}
            <strong className="text-primary">{restaurant?.name}</strong> •
            Linking Road
          </p>
        </div>
        ...
        <button
          onClick={clearCart}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-danger/10 text-danger hover:bg-danger/20 font-bold text-xs transition cursor-pointer"
        >
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-surface p-6 sm:p-8 rounded-3xl border border-border space-y-4 shadow-xs">
            <h2 className="text-lg font-black text-text-primary flex items-center gap-2">
              <ShoppingBag className="text-primary" size={20} /> Order Summary (
                ...
                >
                  ...
                  <div className="flex items-center justify-between xs:justify-end gap-4 w-full xs:w-auto shrink-0 mt-2 xs:mt-0">
                    <div className="flex items-center gap-2 px-2.5 py-1 rounded-xl bg-muted border border-border">
                      ...
                      <span className="font-black text-xs px-1 text-text-primary">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() =>
          ...
          <div className="bg-surface p-6 sm:p-8 rounded-3xl border border-border space-y-4 shadow-xs">
            <h2 className="text-lg font-black text-text-primary flex items-center gap-2">
              <CreditCard className="text-primary" size={20} /> Select Payment
              Method
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                {
                  id: "Wallet",
                  label: `Wallet (₹${user?.walletBalance || 1200})`,
                  icon: "👛",
                },
              ].map((pm) => (
                <button
