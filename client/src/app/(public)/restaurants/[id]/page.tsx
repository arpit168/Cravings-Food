...
export default function RestaurantDetails() {
  ...
  const { items, addItem, updateQuantity, getCartTotals } = useCartStore() as any;
  ...
  return (
    <div className="min-h-screen pb-32 bg-background transition-colors duration-300">
      ...
      <div className="sticky top-16 sm:top-20 z-40 bg-surface/95 backdrop-blur-md border-b border-border shadow-xs">
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredItems.map((item) => {
              const cartItem = items.find((ci: any) => ci._id === item._id);
