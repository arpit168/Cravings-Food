  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8 bg-background transition-colors duration-300">
      <div className="text-center space-y-3 max-w-2xl mx-auto">
        <h1 className="text-3xl sm:text-5xl font-black text-text-primary">
          Explore All Cravings Cuisines
        </h1>
        <p className="text-text-muted text-sm sm:text-base">
          Browse through hundreds of signature gourmet items crafted by top
          partner kitchens.
        </p>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 animate-pulse">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-64 bg-surface rounded-3xl border border-border"
            ></div>
          ))}
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-5 space-y-2">
                <div className="flex justify-between items-center">
                  <h3 className="font-bold text-lg text-text-primary group-hover:text-primary transition">
                    {resItem.name}
                  </h3>
